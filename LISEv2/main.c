...
// creates the directory structure for the progam
/*void setDirectories();
...
void setDirectories() {
    ...
    if (stat("./resutls", &st) == -1) {
        mkdir("./results", 0700);
    }
    //creates details directory
    if (stat("./details", &st) == -1) {
        mkdir("./details", 0700);
    }
...
void checkID(const char *id) {
    struct stat st;
    //creates results directory
    if (stat("./resutls/" + id + , &st) == -1) {
        mkdir("./results", 0700);
    }
    //creates details directory
    if (stat("./details", &st) == -1) {
...
*/
/*
int getinputs(const char *line, ssize_t line_size, char ***restrict input);
int getinput(const char *line, int input_size, char **restrict input);

int main(void) {
  
  fprintf(stdout, "LISE Binding Site Predictions\n");
    ...
    while (1) {
        ...
        numInputs = getinputs(line, lineSize, &inputs);
        printf("num inputs: %d\n", numInputs);
        printf("inputs: %s %s", inputs[0], inputs[1]);
        //getinput(line, numInputs, inputs);
    }
    free(line);
    return 0;
}

int getinputs(const char *line, ssize_t line_size, char ***restrict inputs) {
    int n = 0;
    ...
    *inputs = (char **)realloc(*inputs, (n * sizeof(char*)));
    getinput(line, n, *inputs);
    return n;
...
int getinput(const char *line, int input_size, char **restrict input) {
    ...
    for (int i = 0; i < input_size; i++) {
        sscanf(line, "%s ", str);
        input[i] = (char *)malloc(strlen(str)+1);
        strcpy(*input, str);
        input[i][strlen(str)] = '\0';
    }
    return 1;
}
*/
...
int main(int argc, char *agrv[]) {
    ...
    while (1) {
        ...
        printf("Here");
        num_inputs = getinputs(line_buf, line_buf_size, &inputs);
        printf("Num of inputs: %d\n", num_inputs);
        printf("First Input: %s, Second Input: %s.", inputs[0], inputs[1]);
